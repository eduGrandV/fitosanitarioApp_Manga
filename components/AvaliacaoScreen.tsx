        ...
        <View style={styles.modalContainer}>
          ...
          {status === "buscando" && (
            <View style={styles.loadingContainer}>
              ...
              <Text style={{fontSize: 12, color: '#94a3b8', marginTop: 10}}>Sem internet demora um pouco mais.</Text>
            </View>
          )}
            ...
            <View style={styles.loadingContainer}>
              ...
              <View style={styles.errorButtons}>
                ...
                <TouchableOpacity onPress={onForceLocation} style={[styles.btnBase, {backgroundColor: '#f59e0b', marginTop: 8}]}>
                  <Text style={[styles.btnTextWhite, {color: '#78350f'}]}>📍 Usar Local Padrão</Text>
                </TouchableOpacity>
...
export default function AvaliacaoScreen({
  ...
  const {
      ...
      plantaSelecionada, setPlantaSelecionada,
      filtroSelecionado, setFiltroSelecionado,
      nomeAvaliador, setNomeAvaliador,
      local, 
      handleLocalChange,
  ...
  const [registroPendente, setRegistroPendente] = useState<PendingAction | null>(null);
  const isLockedRef = useRef(false);
  const [tentativasFallback, setTentativasFallback] = useState(0);

  const togglePlantaSelecionada = useCallback((planta: number) => {
    setPlantasSelecionadas((prev) => prev.includes(planta) ? prev.filter((p) => p !== planta) : [...prev, planta]);
  }, []);

  const handlePlantaChange = useCallback((novaPlanta: number) => setPlantaSelecionada(novaPlanta), [setPlantaSelecionada]);
  const handleFiltroChange = useCallback((novaDoenca: string) => setFiltroSelecionado(novaDoenca), [setFiltroSelecionado]);
  ...
  const iniciarFluxoDeRegistro = (action: PendingAction) => {
    ...
    if (!centroCustoSelecionado) {
      Alert.alert("⚠️ Local não selecionado", "Selecione a Fazenda e o Lote primeiro.");
      return;
    }
    if (!nomeAvaliador) {
  ...
  const handleRecalculate = () => { buscarGPS(); };
  ...
  const handleConfirm = () => {
    ...
    const identificadorVariedade = local || null;
    ...
    if (registroPendente.type === 'numeric') {
            ...
            const avaliacoesAtualizadas = prev.filter(
              (r) =>
                !(r.planta === planta && r.doencaOuPraga === doencaOuPraga && r.orgao === orgao &&
                  ...
                  (r.identificadorDeLocal || null) === (localPicker || null) && (r.numeroLocal || null) === (numeroLocal || null))
            );
            
            if (nota != null && nota >= 0) {
              const novoRegistro: Registro = {
                id: Date.now(),
                planta, doencaOuPraga, orgao, quadrante, ramo, 
                nota,
                lote: loteNumeroReal, 
                centroCusto: centroCustoSelecionado, criadoEm: new Date().toISOString(),
                local: pontoConfirmado, localId: pontoConfirmado.id,
                identificadorDeLocal: identificadorVariedade,
        ...
        setAvaliacoes((prev) => {
            ...
            if (existe) {
              ...
              const novoRegistro: Registro = {
                id: Date.now(), planta, doencaOuPraga, orgao, 
                lote: loteNumeroReal, 
                centroCusto: centroCustoSelecionado, nota: 1, criadoEm: new Date().toISOString(),
                local: pontoConfirmado, localId: pontoConfirmado.id,
                identificadorDeLocal: identificadorVariedade, 
                numeroLocal: null, nomeAvaliador,
              };
  ...
  const getCheckboxValue = useCallback(
    (planta: number, doencaOuPraga: string, orgao: string) =>
      avaliacoes.some((a) => a.planta === planta && a.doencaOuPraga === doencaOuPraga && a.orgao === orgao && a.nota === 1),
    [avaliacoes]
  );

  const getNota = useCallback(
    (planta: number, itemNome: string, orgaoNome: string, q: string, r?: string, localItem?: string, cc?: string): number =>
      avaliacoes.find(
        (a) => a.planta === planta && a.doencaOuPraga === itemNome && a.orgao === orgaoNome &&
          (a.quadrante || null) === (q || null) && (a.ramo || null) === (r || null) &&
          (a.identificadorDeLocal || null) === (localItem || null) && a.centroCusto === cc
      )?.nota ?? 0,
    [avaliacoes]
  );

  const itemSelecionado = useMemo(() => doencasPragas.find((d) => d.nome === filtroSelecionado), [filtroSelecionado]);
  const plantasComDados = useMemo(() => Array.from(new Set(avaliacoes.map((a) => a.planta))), [avaliacoes]);
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontSize: 24, fontWeight: "800", color: "#333", marginBottom: 10 },
  ...
  warningTitle: { color: '#b45309', fontWeight: 'bold', fontSize: 16, marginBottom: 4 },
  ...
  label: { fontSize: 14, color: '#64748b', marginBottom: 2, textAlign: 'center' },
  ...
  btnBase: { padding: 16, borderRadius: 12, alignItems: "center", justifyContent: "center", elevation: 2 },
  btnRecalc: { flex: 1, backgroundColor: "#f1f5f9", borderWidth: 1, borderColor: "#cbd5e1" },
