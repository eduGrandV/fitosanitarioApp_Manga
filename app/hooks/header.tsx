...
import { DoencaPraga, doencasPragas, locaPlanta, Registro } from "../../data/daodaAvaliacao";
...
export const useAvaliacaoScreenState = (numeroDePlantas: number) => {
    ...
    const [filtroSelecionado, setFiltroSelecionado] = useState<string>(doencasPragas[0].nome);
    ...
    return {
        ...
        local, setLocal,
        centroCustoSelecionado, setCentroCustoSelecionado,
        plantas,
        handleLocalChange,
        fazendaSelecionada,
        handleFazendaChange,
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.fieldContainer}>
                ...
                <View style={[styles.pickerWrapper, { borderColor: '#2563EB', borderWidth: 2 }]}>
                    <Picker
                    ...
                    >
                        <Picker.Item label="Toque para selecionar..." value="" color="#666" style={{ fontSize: 16 }} />
            ...
            <View style={styles.fieldContainer}>
                ...
                <View style={[
                    ...
                    { opacity: fazendaSelecionada ? 1 : 0.5, backgroundColor: fazendaSelecionada ? '#fff' : '#f3f4f6' }
                ]}>
                    <Picker
                        selectedValue={local}
                        style={styles.picker}
                    ...
                    >
                        <Picker.Item 
                            label={fazendaSelecionada ? "Escolha o lote..." : "Selecione a fazenda acima primeiro ⬆️"} 
                            value={undefined} 
                            color="#666" 
                            style={{ fontSize: 16 }} 
                        />
                        {lotesFiltrados.map((d) => (
                            <Picker.Item key={d.id.toString()} label={d.name} value={d.name} color="#000" style={{ fontSize: 16 }} />
                        ))}
                    </Picker>
                </View>
            </View>
            ...
            <View style={styles.rowContainer}>
                <View style={[styles.fieldContainer, {flex: 1, marginRight: 8}]}>
                    ...
                    <View style={styles.pickerWrapper}>
                        ...
                        >
                            {plantas?.map((p) => (
                                <Picker.Item key={p} label={`P${p}`} value={p} color="#000" style={{ fontSize: 16 }} />
                            ))}
                        </Picker>
                    </View>
                </View>
                ...
                <View style={[styles.fieldContainer, {flex: 1}]}>
                    ...
                    <View style={styles.pickerWrapper}>
                        ...
                        >
                            {doencasPragas.map((d) => (
                                <Picker.Item key={d.nome} label={d.nome} value={d.nome} color="#000" style={{ fontSize: 14 }} />
                            ))}
                        </Picker>
                    </View>
                </View>
            ...
            {itemSelecionado && (
                <View style={styles.statusCard}>
                    <Text style={styles.statusText}>
                        <Text style={{ fontWeight: 'bold' }}>{itemSelecionado.nome}</Text> — Planta {plantaSelecionada}
                    </Text>
                </View>
            )}
        </View>
